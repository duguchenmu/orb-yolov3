...
class ChannelGate(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('mlp input shape:', y.shape)
        y = self.mlp(y)
        # print('bn input shape:', y.shape)
        if y.shape[0] == 1:
            y = y.view(b,c,1,1)
        else:
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.conv2(y)
        y = self.conv3(y)
        # print('bn2d input shape:============:', y.shape)
        y = self.bn(y)
        return y.expand_as(x)
...
class SpatialGate1(nn.Module):
    def __init__(self, channel, kernel_size=3):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.conv = nn.Conv2d(in_channels=channel*2, out_channels=1, kernel_size=kernel_size, padding=padding, bias=False)
        self.sigmoid = nn.Sigmoid()
    def forward(self, x):
        b, c, h, w = x.shape
        avg_pool = self.avgpool(x)
        max_pool = self.maxpool(x)
        # max_pool = torch.mean(x, dim=1, keepdim=True)
...
class BAM(nn.Module):
    def __init__(self, channel):
        ...
        self.spatial_attn = SpatialGate1(channel)

    def forward(self, x):
        # print('BAM=====input shape:', x.shape)
        attn = F.sigmoid( self.spatial_attn(x)+self.channel_attn(x))
        # print('BAM======attn shape:', attn.shape)
        out = x+x*attn
        # print('MAM======output shape:',out.shape)
        return out
